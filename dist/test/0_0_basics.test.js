...
import { Client, Institution, Mutualism, Project, gen_institution, gen_client, gen_project } from './utils/testing_harness.js';
describe('Vitamins Basics', function () {
    function get_setup(institution_database = new Map(), client_database = new Map(), project_database = new Map(), mutualsm_database = new Map()) {
        let collection_mutualism = new Mutualism(['institution', 'client', 'mutualism'], mutualsm_database);
        let collection_project = new Project(['institution', 'project'], project_database);
        let collection_client = new Client(['institution', 'client'], client_database, collection_mutualism);
        let collection_institution = new Institution(['institution'], institution_database, collection_client, collection_project);
        let api = {
    ...
    it(`should do a basic query`, async function () {
        ...
        let { vue, api } = get_setup(institution_database);
    ...
    it(`should do a basic document query that generates a child query`, async function () {
        ...
        let { vue, api } = get_setup(institution_database, client_database);
        let vitamins = new Vitamins(vue);
        vitamins.query(api.collection('institution').document(institution_1._id), undefined, (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), {})).run();
        await sleep(20);
        let test_against = gen_vue();
        test_against.institutions.set(institution_1._id, structuredClone(institution_1));
        test_against.clients.set(client_1._id, structuredClone(client_1));
    ...
    it(`should do a basic query that generates a child query that generates a child query`, async function () {
        ...
        let { vue, api } = get_setup(institution_database, client_database, project_database);
        let vitamins = new Vitamins(vue);
        vitamins.query(api.collection('institution'), {}, (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('project'), { client_id: client_1._id }), (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('project'), { client_id: client_2._id })).run();
        await sleep(20);
        let test_against = gen_vue();
        test_against.institutions.set(institution_1._id, structuredClone(institution_1));
    ...
    it(`should do a basic query that generates child queries accessing the same documents`, async function () {
        ...
        let { vue, api } = get_setup(institution_database, client_database);
        let vitamins = new Vitamins(vue);
        vitamins.query(api.collection('institution'), {}, (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), { _id: client_1._id })).run();
        await sleep(20);
        let test_against = gen_vue();
        test_against.institutions.set(institution_1._id, structuredClone(institution_1));
        test_against.institutions.set(institution_2._id, structuredClone(institution_2));
    ...
    it(`when two identical queries with different children are generated, the query should run only once and both children should run correctly`, async function () {
        ...
        let { vue, api } = get_setup(institution_database, client_database);
        let vitamins = new Vitamins(vue);
        vitamins.query(api.collection('institution')?.document(institution_1._id), undefined, (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), { _id: client_1._id }), (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), { _id: client_2._id })).run();
        await sleep(20);
        vitamins.query(api.collection('institution')?.document(institution_1._id), undefined, (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), { _id: client_2._id }), (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), { _id: client_3._id })).run();
        await sleep(20);
        let test_against = gen_vue();
        test_against.institutions.set(institution_1._id, structuredClone(institution_1));
        test_against.clients.set(client_1._id, structuredClone(client_1));
    ...
    it(`should switch targets when the data changes`, async function () {
        ...
        let { vue, api } = get_setup(institution_database, client_database);
        let vitamins = new Vitamins(vue);
        let query = await vitamins.document(api.collection('institution')?.document('*').collection('client').document(client_1._id), (result) => vitamins.query(api.collection('institution'), { _id: result.institution_id })).run();
        await sleep(20);
    ...
    it(`should handle an external deletion`, async function () {
        ...
        let { vue, api } = get_setup(institution_database, client_database, project_database);
        let vitamins = new Vitamins(vue);
        let query = await vitamins.query(api.collection('institution'), {}, (result) => vitamins.query(api.collection('institution')?.document('*').collection('client'), { institution_id: result._id }, (result) => vitamins.query(api.collection('institution')?.document('*').collection('project'), { client_id: result._id }))).run();
        await sleep(20);
        let test_against_phase_1 = gen_vue();
        test_against_phase_1.institutions.set(institution_1._id, structuredClone(institution_1));
        test_against_phase_1.clients.set(client_1._id, structuredClone(client_1));
    ...
    it(`in a basic query, errors get bubbled up to the parent context`, async function () {
        let institution = gen_institution('test institution');
        let institution_database = database(institution);
        let { vue, api, collection_institution } = get_setup(institution_database);
    ...
    it(`in a basic query that generates a child query, errors get bubbled up to the parent context`, async function () {
        ...
        assert.rejects(async () => {
            let vitamins = new Vitamins(vue);
            await vitamins.query(api.collection('institution'), {}, (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), {})).run();
        }, 'Error: arbitrary error');
    });
    it(`in a basic document query that generates a child query, errors get bubbled up to the parent context`, async function () {
        ...
        assert.rejects(async () => {
            let vitamins = new Vitamins(vue);
            await vitamins.document(api.collection('institution').document(institution_1._id), (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('client'), {})).run();
        }, 'Error: arbitrary error');
    });
    it(`in a basic query that generates a child query that generates a child query, errors get bubbled up to the parent context`, async function () {
        ...
        assert.rejects(async () => {
            let vitamins = new Vitamins(vue);
            await vitamins.query(api.collection('institution'), {}, (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('project'), { client_id: client_1._id }), (result) => vitamins.query(api.collection('institution')?.document(result._id).collection('project'), { client_id: client_2._id })).run();
        }, 'Error: arbitrary error');
    });
    ...
    it(`should handle an unlisten where two child queries point at the same document`, async function () {
        ...
        let query_1 = await vitamins.document(api.collection('institution').document(institution_1._id), (institution) => vitamins.query(api.collection('institution')?.document('*').collection('client'), { institution_id: institution_1._id })).run();
        let query_2 = await vitamins.document(api.collection('institution').document(institution_2._id), (institution) => vitamins.query(api.collection('institution')?.document('*').collection('client'), { institution_id: institution_1._id })).run();
        await sleep(20);
        let test_against_phase_1 = gen_vue();
        test_against_phase_1.institutions.set(institution_1._id, structuredClone(institution_1));
...
//# sourceMappingURL=0_0_basics.test.js.map