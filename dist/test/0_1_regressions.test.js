...
describe('Bug Regressions', function () {
    ...
    it(`a sweeping query should not block a specific query's child queries`, async function () {
        ...
        let { vue, api } = get_setup(institution_database, client_database, project_database, mutualism_database);
        let vitamins = new Vitamins(vue);
        vitamins.query(api.collection('institution')?.document(institution_1._id).collection('client'), {}, (client) => vitamins.document(api.collection('institution')?.document(institution_1._id).collection('project').document(project_1._id))).run();
        await sleep(20);
        vitamins.query(api.collection('institution'), {}, (institution) => {
            return vitamins.document(api.collection('institution').document(institution_1._id).collection('project').document(project_1._id), (project) => { return vitamins.query(api.collection('institution').document(institution_1._id).collection('client').document(client_1._id).collection('mutualism'), undefined); });
        }).run();
        await sleep(20);
    ...
    it(`a document query that does not generate children should not fail`, async function () {
        ...
        let vitamins = new Vitamins(vue);
        let query_1 = await vitamins.document(api.collection('institution').document('irrelevant_doc'), (institution) => vitamins.document(api.collection('institution').document(institution._id).collection('client').document(client_1._id))).run();
        await sleep(20);
    });
});
//# sourceMappingURL=0_1_regressions.test.js.map