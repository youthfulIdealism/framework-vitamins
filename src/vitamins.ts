...
class QuerySpec extends QueryShape {
    ...
    async run(): Promise<{ query: Query, get_results: Query['get_results'], rerun: Query['rerun'], unlisten: () => void }> {
        ...
        // set up a new root Link for the query
        let root_link = new Link();
        vitamins.roots.add(root_link);

        // if this query already exists, attach it to this 
        // new root link instead of doing a refetch
        ...
        // return context that can be used by the frontend caller
        return {
            query,
...
export class Vitamins {
    ...
    /*
        ...
        rewalking holds the queries being re-walked further up the (synchronous) call stack.
    */
    _resolve_query(query_spec: QuerySpec, link: Link, loop_detector: ReadonlySet<Query> = new Set()): { query: Query, fetch?: Promise<void> } {
        // find out if the query has already been generated by some other call.
        let self = this._find_existing_query(query_spec);
        let is_new = !self;
        if(!self) {
            // if it hasn't, generate a new Query from the query spec and use it.
        ...
        // ...otherwise, if we can use an existing query,...
        if(link.query !== self) {
            // cut the ties between the link and whatever query it was attached to before,
            // and--since the link's generators came from the query we just unlinked,
            // clear the list of generators contributed by the link.
            if(link.query) {
        ...
        // set the generator contribued by the link to what is specified by the query_spec
    ...
    */
    _set_generators_contributed_by_link(link: Link, generator_functions: child_generator<result>[]) {
        let added: Generator[] = [];

        // add and remove generators as needed
    ...
    // Returns the link, if the generator produced anything.
    _run_generator(document: Document, generator: Generator, rewalking: ReadonlySet<Query> = new Set()) {
    ...
    _update_data(reference: generated_collection_interface<result> | generated_document_interface<result> | undefined, document_id: string, data: result, query?: Query, collect_garbage: boolean = true) {
        ...
        let refreshed_links = new Set<Link>();
        for(let parent_query of Array.from(document.parents)) {
            for(let generator of Array.from(parent_query.generators.values())) {
                ...
                if(link) { refreshed_links.add(link); }
            }
        }

        // any previous link this didn't refresh is stale: its generator is gone or no longer runs here.
        for(let link of previous_links) {
            if(!refreshed_links.has(link)) { this._remove_link(link); }
        }
